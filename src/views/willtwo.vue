        ...
        <el-table-column align="left" label="操作" width="200px">
          <template #default="scope" >
            ...
            <el-button  color="#d11579" style="color: #fff;"
                type="danger" v-if="scope.row.status=='已驳回'"
...
import { v4 as uuid4 } from 'uuid'
  ...
  export default {
    ...
    methods: {
        ...
        axios.post(`http://localhost:9099/willtwo/findStudents/${this.wills[index].studentidcard}` ).then((res) => {
       ...
       getWills() {
          axios.get('http://localhost:9099/willtwo/findAll').then(res => {
              // 处理返回的数据
              console.log(res.data);
      ...
      handleDelete(index, scope){
          ...
          ElMessageBox.confirm(
            ...
            .then(() => {
              axios.delete(`http://localhost:9099/willtwo/deleteById/${scope.id}`).then(() => {
                this.getWills()
              })
              ElMessage({
                type: 'success',
      ...
      submitEditForm(formEl){
        ...
        axios.post(`http://localhost:9099/willtwo/applychoice`, this.willform).then((res) => {
      ...
      searchResult(){
        ...
        axios.post(`http://localhost:9099/willtwo/searchWill`, this.willform).then((res) => {
